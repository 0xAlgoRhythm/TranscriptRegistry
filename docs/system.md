...
1. **`UniversityFactoryBeacon.sol`**:
   - Acts as the registry deployment hub using the Upgradeable Beacon pattern.
   - Leverages a multi-tenant factory pattern where every university gets its own isolated `TranscriptRegistryUpgradeable` proxy instance.
   - Emits `UniversityDeployed` which serves as the indexer trigger.
2. **`TranscriptRegistryUpgradeable.sol`**:
