...
import { ShieldCheck, FileText, Mail, Download, Key, Lock, ArrowLeft, Building2, HelpCircle, AlertCircle, CheckCircle, Send, Loader2, Clock, Search } from "lucide-react";
  ...
  return <div className="flex min-h-screen flex-col items-center bg-background text-foreground p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-3xl space-y-6 relative z-10 my-10 animate-fade-in">
        ...
        <div className="flex justify-between items-center">
          <button onClick={() => router.push("/verify")} className="inline-flex items-center gap-1.5 text-xs font-mono tracking-wider text-muted-foreground hover:text-foreground transition-all">
            <ArrowLeft className="h-4 w-4" />{t("bACKTOSEARCH")}</button>
          
          <div className="flex items-center gap-1 text-[10px] font-mono text-muted-foreground uppercase">
            <span>{t("pUBLICACCESSAUDITPORTAL")}</span>
          </div>
        </div>

        {/* Loading Spinner */}
        {loading && <div className="text-center py-24 font-mono text-xs text-muted-foreground animate-pulse flex flex-col items-center gap-3">
            <Loader2 className="h-8 w-8 text-ca-accent animate-spin" />
            <span>{t("rETRIEVINGLEDGERSTATUSAND")}</span>
          </div>}

        {/* Error State */}
        {!loading && error && <GlowCard className={`p-8 text-center space-y-4 border ${errorCode === "EXPIRED_TOKEN" ? "border-ca-warning/25 bg-ca-warning/5" : errorCode === "NOT_FOUND" ? "border-muted/25 bg-muted/5" : "border-ca-danger/25 bg-ca-danger/5"}`}>
            {errorCode === "EXPIRED_TOKEN" ? <Clock className="h-10 w-10 text-ca-warning mx-auto animate-pulse" /> : errorCode === "NOT_FOUND" ? <Search className="h-10 w-10 text-muted-foreground mx-auto" /> : <AlertCircle className="h-10 w-10 text-ca-danger mx-auto animate-pulse" />}
            <h2 className={`text-lg font-mono font-bold uppercase tracking-wider ${errorCode === "EXPIRED_TOKEN" ? "text-ca-warning" : errorCode === "NOT_FOUND" ? "text-foreground" : "text-ca-danger"}`}>
              {errorCode === "EXPIRED_TOKEN" ? "ACCESS TOKEN EXPIRED" : errorCode === "NOT_FOUND" ? "RECORD NOT FOUND" : errorCode === "NETWORK_ERROR" ? "NETWORK CONNECTION FAILED" : "VERIFICATION FAILED"}
            </h2>
            <p className="text-xs text-muted-foreground leading-relaxed max-w-sm mx-auto">
              {error}
            </p>
            {errorCode === "EXPIRED_TOKEN" && <p className="text-[10px] text-muted-foreground mt-2">{t("pleaseaskthestudent")}</p>}
            {errorCode === "NETWORK_ERROR" && <Button onClick={() => verifyRecord()} variant="outline" size="sm" className="mt-4 font-mono text-xs">{t("retryConnection")}</Button>}
          </GlowCard>}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs text-muted-foreground">
                      ...
                      <div className="space-y-1">
                        ...
                        <span className="text-foreground bg-muted/20 px-2 py-1 rounded border border-border/30 block">
                          {typeof result.transcript?.issuedAt === 'string' ? new Date(result.transcript.issuedAt).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "short",
                      day: "numeric"
