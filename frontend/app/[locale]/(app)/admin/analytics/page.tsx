...
export default function AdminAnalyticsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-10 animate-fade-in pb-16">
      ...
      <div className="space-y-1">
        ...
        <h1 className="text-3xl font-mono font-bold tracking-tight uppercase text-foreground">{t("platformAnalytics")}</h1>
        <p className="text-xs text-muted-foreground">{t("aggregateplatformstatisticsdeployment")}</p>
      </div>

      {/* Metrics overview */}
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 font-mono">
        <StatCard label="Total Issued Records" value={dbStats ? String(dbStats.totalTranscripts) : "..."} icon={<BarChart className="h-5 w-5" />} accent="default" trend="Live database count" />
        <StatCard label="Verification Requests" value={dbStats ? String(dbStats.totalVerifications) : "..."} icon={<ShieldCheck className="h-5 w-5" />} accent="success" trend="Successful audits" />
        <StatCard label="Active Universities" value={dbStats ? String(dbStats.activeUniversities) : "..."} icon={<Landmark className="h-5 w-5" />} accent="teal" trend={`${(dbStats?.totalUniversities || 0) - (dbStats?.activeUniversities || 0)} suspended`} />
        <StatCard label="Verification Volume" value={dbStats ? `${((dbStats.totalVerifications || 0) * 1.0).toFixed(1)}` : "..."} icon={<TrendingUp className="h-5 w-5" />} accent="success" trend="Dynamic load" />
      </div>
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <GlowCard className="p-6 h-64 flex flex-col justify-between" glow>
            <div className="flex justify-between items-center border-b border-border/40 pb-3 font-mono text-xs">
              <span className="font-bold">{t("vERIFICATIONVELOCITYHISTORY")}</span>
              <span className="text-muted-foreground">{t("30DAYTIMELINE")}</span>
            </div>
            
            {/* Visual mock chart bars */}
            <div className="flex items-end gap-2 h-40 pt-4 px-2">
              {[20, 35, 45, 30, 25, 40, 55, 65, 50, 45, 60, 75, 80, 65, 55, 70, 85, 90, 80, 75, 95, 110, 100, 90, 105, 120, 130, 115, 105, 125].map((val, idx) => <div key={idx} className="flex-grow bg-ca-accent/65 hover:bg-ca-accent transition-colors rounded-t" style={{
              height: `${val / 130 * 100}%`
